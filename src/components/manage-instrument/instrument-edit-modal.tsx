                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    <div className="grid md:grid-cols-2 gap-x-6 gap-y-4 p-4 border rounded-md">
                        ...
                        <div className="space-y-4">
                            <FormField name="mandateId" control={form.control} render={({ field }) => ( <FormItem><FormLabel>Mandate ID</FormLabel><Input {...field} readOnly disabled/></FormItem> )}/>
                            <FormField name="category" control={form.control} render={({ field }) => ( <FormItem><FormLabel>Category</FormLabel><Input {...field} readOnly disabled/></FormItem> )}/>
                        ...
                        <div className="space-y-4">
                            ...
                            <FormField name="placedDate" control={form.control} render={({ field }) => (<FormItem className="flex flex-col"><FormLabel>Placed Date</FormLabel><Popover><PopoverTrigger asChild><Button variant={"outline"} className={cn("pl-3 text-left font-normal", !field.value && "text-muted-foreground")}><CalendarIcon className="mr-2 h-4 w-4" />{field.value ? format(field.value, "PPP") : <span>Enter Date</span>}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={field.value || undefined} onSelect={field.onChange} /></PopoverContent></Popover><FormMessage /></FormItem>)}/>
                                ...
                                <TableBody>
                                    {allInstruments.map(inst => (
                                        <TableRow key={inst.id}>
                                            ...
                                            <TableCell>{inst.ratingAnalyst}</TableCell>
                                            <TableCell>{new Date(inst.mandateDate).toLocaleString()}</TableCell>
                                            <TableCell>{inst.groupHead}</TableCell>
                                            <TableCell>{new Date(inst.mandateDate).toLocaleString()}</TableCell>
                                            <TableCell>
