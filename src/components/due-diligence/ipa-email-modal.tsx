...
import type { RatingNote, IPA, IPADiscussion, AppUser } from '@/lib/definitions';
...
interface IpaEmailModalProps {
    isOpen: boolean;
    onClose: () => void;
    note: RatingNote;
    ipa: IPA;
    discussion: IPADiscussion;
    analyst: AppUser;
}

const generateFeedbackSeekingBody = (note: RatingNote, ipa: IPA, analyst: AppUser): string => {
...
const generateMinutesConfirmationBody = (note: RatingNote, ipa: IPA, analyst: AppUser, discussion: IPADiscussion): string => {
    let body = `Dear Sir/Madam,\n\n`;
...
export function IpaEmailModal({ isOpen, onClose, note, ipa, discussion, analyst }: IpaEmailModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            const emailBody = discussion.discussionHappened === 'Yes'
                ? generateMinutesConfirmationBody(note, ipa, analyst, discussion)
                : generateFeedbackSeekingBody(note, ipa, analyst);
    ...
    }, [isOpen, note, ipa, discussion, analyst, reset, groupHead]);
    ...
    const onSubmit = (data: EmailFormValues) => {
        console.log('Sending IPA email:', data);
        toast({
            title: 'Email Sent',
      ...
      <>
        <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
            <DialogContent className="sm:max-w-4xl">
                <DialogHeader>
                    <DialogTitle>IPA Email Confirmation</DialogTitle>
                </DialogHeader>
                        ...
                        <div className="grid grid-cols-[100px_1fr] items-center gap-4 px-4">
                            <Label className="text-right text-muted-foreground">Attachments</Label>
                             <div className="flex items-center gap-2 p-2 border rounded-md bg-muted text-sm">
                                ...
                                <span>{`IPA_Questionnaire_${note.companyName.replace(/ /g, '_')}.pdf`}</span>
                            </div>
                        </div>
                         <div className="px-4">
                            <Label>Body</Label>
