...
interface DtaDiscussionTableProps {
    ...
    dta: DTA;
    note: RatingNote;
    onAddDiscussion: (newDiscussion: DTADiscussion) => void;
}
...
const statusVariant = (status: DTADiscussion['status']) => {
    switch (status) {
...
export function DtaDiscussionTable({ discussions, dta, note, onAddDiscussion }: DtaDiscussionTableProps) {
    ...
    // Placeholder contacts for DTAs
     const mockDtaContacts = [
        { id: 'dta-contact-1', name: 'Amit Sharma', email: 'amit@abc.com', contact: '0000000000' },
        { id: 'dta-contact-2', name: 'Vijay Varma', email: 'vijay@abc.com', contact: '0000000000' },
    ];


    const handleAdd = (data: DiscussionFormValues) => {
        const contact = mockDtaContacts.find(c => c.name === data.contactPerson);
        if (!contact) return;

        const newDiscussion: DTADiscussion = {
            id: uuidv4(),
            ...data,
            minutesCaptured: 'No',
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            <Select onValueChange={field.onChange} value={field.value}>
                                                ...
                                                <SelectContent>
                                                    {mockDtaContacts.map(c => <SelectItem key={c.id} value={c.name}>{c.name}</SelectItem>)}
                                                </SelectContent>
                                            </Select>
                                        )}
                                    />
