...
export function BankerFeedbackAccordion({ firms, companyId }: BankerFeedbackAccordionProps) {
    ...
    const [activeContact, setActiveContact] = useState<BankerContact | null>(null);
    const [activeFirm, setActiveFirm] = useState<BankerFirm | null>(null);
    ...
    const handleCreateRecord = async (firmId: string) => {
        ...
        try {
            await fetch('/api/due-diligence/banker-feedback', {
                method: 'POST',
            ...
            mutate(`/api/due-diligence/banker-feedback?companyId=${companyId}`);
            toast({ title: 'Success', description: 'Banker discussion record created.' });
    ...
    const openUploadModal = (contact: BankerContact) => {
        setActiveContact(contact);
        setUploadModalOpen(true);
    };

    const openEmailModal = (firm: BankerFirm, contact: BankerContact) => {
        setActiveFirm(firm);
        setActiveContact(contact);
                                                ...
                                                <TableCell className="text-right">
                                                    <div className="flex gap-1 justify-end">
                                                        <Button variant="ghost" size="icon" asChild disabled={contact.discussionHappened !== 'Yes'}>
                                                            <Link href={`/due-diligence/banker-feedback/${companyId}/${firm.id}/${contact.id}`}>
            ...
            <Dialog open={isUploadModalOpen} onOpenChange={setUploadModalOpen}>
                 <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Attach a document related to the Banker discussion.
                        </DialogDescription>
                    </DialogHeader>
