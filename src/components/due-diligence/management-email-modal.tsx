...
import type { CompanyDashboard, AppUser, DiscussionMinute, ManagementPersonnel } from '@/lib/definitions';
...
interface ManagementEmailModalProps {
    isOpen: boolean;
    onClose: () => void;
...
const generateEmailBody = (discussion: ManagementEmailModalProps['discussion']): string => {
    let body = `Dear Sir/Madam,\n\n`;
    body += `This is with reference to the discussion held on ${discussion.interactionDate ? new Date(discussion.interactionDate).toLocaleDateString() : 'a recent date'}.\n\n`;
...
export function ManagementEmailModal({ isOpen, onClose, company, discussion, analyst }: ManagementEmailModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            reset({
                to: toEmail,
                cc: ccEmails,
                subject: `Minutes of discussion with management of ${company.companyName}`,
                body: emailBody,
            });
        }
    ...
    const onSubmit = (data: EmailFormValues) => {
        console.log('Sending Management Discussion email:', data);
        toast({
            title: 'Email Sent',
