...
interface IpaDiscussionTableProps {
    ...
    ipa: IPA;
    note: RatingNote;
    onAddDiscussion: (newDiscussion: IPADiscussion) => void;
}
...
const statusVariant = (status: IPADiscussion['status']) => {
    switch (status) {
...
export function IpaDiscussionTable({ discussions, ipa, note, onAddDiscussion }: IpaDiscussionTableProps) {
    ...
    const [selectedDiscussion, setSelectedDiscussion] = useState<IPADiscussion | null>(null);
    ...
    const handleAdd = (data: DiscussionFormValues) => {
        ...
        const newDiscussion: IPADiscussion = {
            id: uuidv4(),
            ...data,
            minutesCaptured: 'No',
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            <Select onValueChange={field.onChange} value={field.value}>
                                                ...
                                                <SelectContent>
                                                    {mockIpaContacts.map(c => <SelectItem key={c.id} value={c.name}>{c.name}</SelectItem>)}
                                                </SelectContent>
                                            </Select>
                                        )}
                                    />
