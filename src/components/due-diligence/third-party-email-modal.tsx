...
import type { CompanyDashboard, AppUser, ThirdPartyDiscussion } from '@/lib/definitions';
...
interface ThirdPartyEmailModalProps {
    isOpen: boolean;
    onClose: () => void;
    company: CompanyDashboard | null;
    discussion: ThirdPartyDiscussion;
    analyst: AppUser;
}

const generateEmailBody = (discussion: ThirdPartyDiscussion): string => {
    let body = `Dear Sir/Madam,\n\n`;
    body += `This is with reference to the discussion held with ${discussion.organizationName} on ${discussion.interactionDate ? new Date(discussion.interactionDate).toLocaleDateString() : 'a recent date'}.\n\n`;
    body += `The following personnel were present:\n`;
    discussion.personnel?.forEach(p => {
        body += `- ${p.name}, ${p.designation}\n`;
    });
    body += `\nKey points discussed:\n`;
    discussion.minutes?.forEach(m => {
        body += `\nQuery: ${m.query}\nResponse: ${m.response}\n`;
    });
...
export function ThirdPartyEmailModal({ isOpen, onClose, company, discussion, analyst }: ThirdPartyEmailModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            const emailBody = generateEmailBody(discussion);
            ...
            reset({
                to: toEmail,
                cc: ccEmails,
                subject: `Minutes of third-party discussion for ${company?.companyName || 'the company'}`,
                body: emailBody,
            });
        }
    }, [isOpen, company, discussion, analyst, reset, groupHead]);
    ...
    const onSubmit = (data: EmailFormValues) => {
        console.log('Sending Third Party Discussion email:', data);
        toast({
            title: 'Email Sent',
