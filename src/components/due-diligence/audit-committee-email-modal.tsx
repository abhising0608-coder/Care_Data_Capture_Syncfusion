...
import type { CompanyDashboard, AppUser, AuditCommitteeDiscussion } from '@/lib/definitions';
...
interface AuditCommitteeEmailModalProps {
    ...
    discussion: AuditCommitteeDiscussion;
    analyst: AppUser;
}

const generateEmailBody = (discussion: AuditCommitteeDiscussion): string => {
    let body = `Dear Sir/Madam,\n\n`;
    body += `This is with reference to the discussion held with the Audit Committee on ${discussion.interactionDate ? new Date(discussion.interactionDate).toLocaleDateString() : 'a recent date'}.\n\n`;
...
export function AuditCommitteeEmailModal({ isOpen, onClose, company, discussion, analyst }: AuditCommitteeEmailModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            reset({
                to: toEmail,
                cc: ccEmails,
                subject: `Minutes of Audit Committee discussion for ${company?.companyName || 'the company'}`,
                body: emailBody,
            });
        }
    ...
    const onSubmit = (data: EmailFormValues) => {
        console.log('Sending Audit Committee Discussion email:', data);
        toast({
            title: 'Email Sent',
