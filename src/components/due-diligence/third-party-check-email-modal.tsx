...
import type { CompanyDashboard, ThirdPartyCheck, ThirdParty } from '@/lib/definitions';
...
interface ThirdPartyCheckEmailModalProps {
    isOpen: boolean;
    onClose: () => void;
    checkData: Partial<ThirdPartyCheck>;
    company: CompanyDashboard;
    thirdParty: ThirdParty;
}

const generateEmailBody = (checkData: Partial<ThirdPartyCheck>, company: CompanyDashboard, thirdParty: ThirdParty): string => {
    let body = `Dear Team,\n\nPlease find the minutes of our recent discussion regarding ${company.companyName} for your confirmation.\n\n`;

    body += `--------------------------------\n`;
    body += `MEETING DETAILS\n`;
    body += `--------------------------------\n`;
    if (checkData.interactionDate) {
         body += `Date of Interaction: ${format(checkData.interactionDate, "PPP")}\n`;
    }
    body += `Third-Party Organization: ${thirdParty.name}\n`;
    body += `Relationship: ${thirdParty.relationship}\n`;
    body += `Location: ${checkData.location || 'N/A'}\n\n`;

    if (checkData.thirdPartyPersonnel && checkData.thirdPartyPersonnel.length > 0) {
        body += `Attendees (Third Party):\n`;
        checkData.thirdPartyPersonnel.forEach(p => {
            body += `- ${p.name} (${p.designation})\n`;
        });
        body += `\n`;
    }
    
     if (checkData.careTeam && checkData.careTeam.length > 0) {
        body += `Attendees (CARE Team):\n`;
        checkData.careTeam.forEach(member => {
            body += `- ${member}\n`;
        });
        body += `\n`;
    }

    if (checkData.discussionMinutes && checkData.discussionMinutes.length > 0) {
        body += `--------------------------------\n`;
        body += `MINUTES OF DISCUSSION\n`;
        body += `--------------------------------\n`;
        checkData.discussionMinutes.forEach((minute, index) => {
            body += `Query #${index + 1}: ${minute.query}\n`;
...
export function ThirdPartyCheckEmailModal({ isOpen, onClose, checkData, company, thirdParty }: ThirdPartyCheckEmailModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            const primaryContact = thirdParty.contacts.find(c => c.isPrimary);
            const ccList = [user?.email, 'group.head@careedge.com', 'rating.head@careedge.com'].filter(Boolean).join(', ');
            const emailBody = generateEmailBody(checkData, company, thirdParty);
            
            reset({
                to: primaryContact?.email || 'primary.contact@thirdparty.com',
                cc: ccList,
                subject: `Minutes of Discussion: Credit rating of ${company.companyName}`,
                body: emailBody,
            });
        }
    }, [isOpen, checkData, company, thirdParty, user, reset]);
    ...
    const onSubmit = (data: EmailFormValues) => {
        console.log('Sending third-party check minutes email:', data);
        toast({
            title: 'Email Sent',
      ...
      <>
        <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
            <DialogContent className="sm:max-w-3xl">
                <DialogHeader>
                    <DialogTitle>Email Confirmation: Third Party Check</DialogTitle>
                     <DialogDescription>
                        Review and send the minutes of discussion to the third party.
                    </DialogDescription>
                </DialogHeader>
