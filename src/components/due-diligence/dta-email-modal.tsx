...
import type { RatingNote, DTA, DTADiscussion, AppUser } from '@/lib/definitions';
...
interface DtaEmailModalProps {
    isOpen: boolean;
    onClose: () => void;
    note: RatingNote;
    dta: DTA;
    discussion: DTADiscussion;
    analyst: AppUser;
}

const generateFeedbackSeekingBody = (note: RatingNote, dta: DTA, analyst: AppUser): string => {
    let body = `Dear Sir/Madam,\n\n`;
    body += `As part of our annual surveillance of the ratings assigned to the debt instruments of ${note.companyName}, we would appreciate your feedback on the company.\n\n`;
...
const generateMinutesConfirmationBody = (note: RatingNote, dta: DTA, analyst: AppUser, discussion: DTADiscussion): string => {
    let body = `Dear Sir/Madam,\n\n`;
...
export function DtaEmailModal({ isOpen, onClose, note, dta, discussion, analyst }: DtaEmailModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            const emailBody = discussion.discussionHappened === 'Yes'
                ? generateMinutesConfirmationBody(note, dta, analyst, discussion)
                : generateFeedbackSeekingBody(note, dta, analyst);
    ...
    }, [isOpen, note, dta, discussion, analyst, reset, groupHead]);
    ...
    const onSubmit = (data: EmailFormValues) => {
        console.log('Sending DTA email:', data);
        toast({
            title: 'Email Sent',
      ...
      <>
        <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
            <DialogContent className="sm:max-w-4xl">
                <DialogHeader>
                    <DialogTitle>DTA Email Confirmation</DialogTitle>
                </DialogHeader>
                        ...
                        <div className="grid grid-cols-[100px_1fr] items-center gap-4 px-4">
                            <Label className="text-right text-muted-foreground">Attachments</Label>
                             <div className="flex items-center gap-2 p-2 border rounded-md bg-muted text-sm">
                                ...
                                <span>{`DTA_Questionnaire_${note.companyName.replace(/ /g, '_')}.pdf`}</span>
                            </div>
                        </div>
                         <div className="px-4">
                            <Label>Body</Label>
