...
import type { RatingNote, Auditor, AuditorDiscussion, AppUser } from '@/lib/definitions';
...
interface AuditorEmailModalProps {
    ...
    note: RatingNote;
    auditor: Auditor;
    discussion: AuditorDiscussion;
    analyst: AppUser;
}

const generateFeedbackSeekingBody = (note: RatingNote, auditor: Auditor, analyst: AppUser): string => {
...
const generateMinutesConfirmationBody = (note: RatingNote, auditor: Auditor, analyst: AppUser, discussion: AuditorDiscussion): string => {
    let body = `Dear Sir/Madam,\n\n`;
    body += `This is in reference to our discussion on ${discussion.interactionDate ? new Date(discussion.interactionDate).toLocaleDateString() : 'a recent date'} regarding the rating of ${note.companyName}.\n\n`;
...
export function AuditorEmailModal({ isOpen, onClose, note, auditor, discussion, analyst }: AuditorEmailModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            const emailBody = discussion.discussionHappened === 'Yes'
                ? generateMinutesConfirmationBody(note, auditor, analyst, discussion)
                : generateFeedbackSeekingBody(note, auditor, analyst);
            
            const toEmail = discussion.email;
    ...
    }, [isOpen, note, auditor, discussion, analyst, reset, groupHead]);
    ...
    const onSubmit = (data: EmailFormValues) => {
        console.log('Sending Auditor email:', data);
        toast({
            title: 'Email Sent',
      ...
      <>
        <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
            <DialogContent className="sm:max-w-4xl">
                <DialogHeader>
                    <DialogTitle>Auditor Email Confirmation</DialogTitle>
                </DialogHeader>
                        ...
                        <div className="grid grid-cols-[100px_1fr] items-center gap-4 px-4">
                            <Label className="text-right text-muted-foreground">Attachments</Label>
                             <div className="flex items-center gap-2 p-2 border rounded-md bg-muted text-sm">
                                ...
                                <span>{`Auditor_Questionnaire_${note.companyName.replace(/ /g, '_')}.pdf`}</span>
                            </div>
                        </div>
                         <div className="px-4">
                            <Label>Body</Label>
