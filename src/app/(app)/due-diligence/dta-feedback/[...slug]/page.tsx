...
export default function DtaFeedbackCapturePage() {
  const router = useRouter();
  const params = useParams();
  const { toast } = useToast();
  const { mutate } = useSWRConfig();
  const { user } = useAuth();
  const [ratingCycleId, dtaId, discussionId] = params.slug as string[];

  const [isEmailModalOpen, setIsEmailModalOpen] = useState(false);

  const { data: note, isLoading: isNoteLoading } = useSWR<RatingNote>(`/api/notes/${ratingCycleId}`, fetcher);
  const { data: dtas, isLoading: isDtaLoading } = useSWR<DTA[]>(`/api/dta/${ratingCycleId}`, fetcher);
  const { data: discussion, isLoading: isDiscussionLoading } = useSWR<DTADiscussion>(`/api/dta/${ratingCycleId}/${dtaId}/${discussionId}`, fetcher);
  
  const dta = dtas?.find(a => a.id === dtaId);
  ...
  const handleSave = async (data: FeedbackFormValues) => {
    try {
      ...
      await fetch(`/api/dta/${ratingCycleId}/${dtaId}/${discussionId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      mutate(`/api/dta/${ratingCycleId}/${dtaId}/${discussionId}`);
  ...
  const isLoading = isNoteLoading || isDtaLoading || isDiscussionLoading || !user;
  ...
  return (
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-muted-foreground">{dta?.firmName} - {discussion?.contactPerson}</p>
      </header>
                ...
                <div className="rounded-t-lg border">
                    <Table>
                        <TableHeader>
                         ...
                         <TableBody>
                            {mockDtaQuestionnaire.map((q, index) => (
                                <TableRow key={q.id}>
