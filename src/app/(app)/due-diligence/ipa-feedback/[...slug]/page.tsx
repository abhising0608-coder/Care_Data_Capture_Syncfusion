...
export default function IpaFeedbackCapturePage() {
  const router = useRouter();
  const params = useParams();
  const { toast } = useToast();
  const { mutate } = useSWRConfig();
  const { user } = useAuth();
  const [ratingCycleId, ipaId, discussionId] = params.slug as string[];

  const [isEmailModalOpen, setIsEmailModalOpen] = useState(false);

  const { data: note, isLoading: isNoteLoading } = useSWR<RatingNote>(`/api/notes/${ratingCycleId}`, fetcher);
  const { data: ipas, isLoading: isIpaLoading } = useSWR<IPA[]>(`/api/ipa/${ratingCycleId}`, fetcher);
  const { data: discussion, isLoading: isDiscussionLoading } = useSWR<IPADiscussion>(`/api/ipa/${ratingCycleId}/${ipaId}/${discussionId}`, fetcher);
  
  const ipa = ipas?.find(a => a.id === ipaId);
  ...
  useEffect(() => {
    if (discussion) {
      form.reset({
        remarks: (discussion as any).feedback || {},
  ...
  const handleSave = async (data: FeedbackFormValues) => {
    try {
      ...
      await fetch(`/api/ipa/${ratingCycleId}/${ipaId}/${discussionId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      mutate(`/api/ipa/${ratingCycleId}/${ipaId}/${discussionId}`);
  ...
  const isLoading = isNoteLoading || isIpaLoading || isDiscussionLoading || !user;
  ...
  return (
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-muted-foreground">{ipa?.firmName} - {discussion?.contactPerson}</p>
      </header>
                ...
                <div className="rounded-t-lg border">
                    <Table>
                        <TableHeader>
                         ...
                         <TableBody>
                            {mockIpaQuestionnaire.map((q, index) => (
                                <TableRow key={q.id}>
