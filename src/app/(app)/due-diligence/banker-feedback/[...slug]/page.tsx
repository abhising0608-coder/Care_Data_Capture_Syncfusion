...
const defaultQuestionnaire: QuestionnaireItem[] = [
    { srNo: 1, particulars: "Details of defaults by the Company in payment of interest/principal against any of the bank facilities during the last three years.", remarks: "" },
...
export default function BankerFeedbackCapturePage() {
    ...
    const { data, error, isLoading } = useSWR<{ firm: BankerFirm, contact: BankerContact }>(
        companyId && firmId && contactId ? `/api/due-diligence/banker-feedback?companyId=${companyId}&firmId=${firmId}&contactId=${contactId}` : null,
        fetcher
    );
                 ...
                 <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        <h1 className="text-xl font-bold tracking-tight text-foreground">
                            Banker Feedback Capture: {firm.firmName}
                        </h1>
                         ...
                         {!isReadOnly && (
                            <>
                                <Button type="button" variant="outline" onClick={() => setIsEmailModalOpen(true)}>
                                    <Mail className="mr-2 h-4 w-4" />Email to Banker
                                </Button>
