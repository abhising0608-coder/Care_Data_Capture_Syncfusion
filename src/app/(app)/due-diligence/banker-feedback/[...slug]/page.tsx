...
export default function BankerFeedbackCapturePage() {
  ...
  const { data: discussion, isLoading: isDiscussionLoading } = useSWR<BankerDiscussion>(`/api/bankers/${ratingCycleId}/${bankerId}/${discussionId}`, fetcher);
  
  const banker = bankers?.find(a => a.id === bankerId);
  ...
  useEffect(() => {
    if (discussion) {
      form.reset({
        remarks: (discussion as any).feedback || {}, // Assuming feedback is stored on discussion
        summary: (discussion as any).summary || ''
      });
    }
  }, [discussion, form]);
  ...
  const handleSave = async (data: FeedbackFormValues) => {
    try {
      ...
      await fetch(`/api/bankers/${ratingCycleId}/${bankerId}/${discussionId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      mutate(`/api/bankers/${ratingCycleId}/${bankerId}/${discussionId}`); // Revalidate SWR
  ...
  const handleMarkAsComplete = async () => {
    const data = form.getValues();
     try {
      ...
      mutate(`/api/bankers/${ratingCycleId}/${bankerId}/${discussionId}`);
  ...
  const isLoading = isNoteLoading || isBankerLoading || isDiscussionLoading || !user;
  ...
  return (
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-muted-foreground">{banker?.bankName} - {discussion?.contactPerson}</p>
      </header>
                ...
                <div className="rounded-t-lg border">
                    <Table>
                        <TableHeader>
                         ...
                         <TableBody>
                            {mockAuditorQuestionnaire.map((q, index) => ( // Reusing questionnaire
                                <TableRow key={q.id}>
