...
import type { AuditCommitteePersonnel, AuditCommitteeMinute, CompanyDashboard, AuditCommitteeMeeting } from '@/lib/definitions';
...
import { AuditCommitteeEmailModal } from '@/components/due-diligence/audit-committee-email-modal';
...
const formSchema = z.object({
    ...
    auditCommitteePersonnel: z.array(z.object({
        id: z.string(),
...
export default function AuditCommitteeMeetingPage({ isEmbedded = false }: AuditCommitteeMeetingPageProps) {
    ...
    const form = useForm<AuditCommitteeFormValues>({
    ...
    const [newPersonnel, setNewPersonnel] = useState<Partial<AuditCommitteePersonnel>>({});
    
    const [editingMinuteIndex, setEditingMinuteIndex] = useState<number | null>(null);
    const [newMinute, setNewMinute] = useState<Partial<AuditCommitteeMinute>>({});

    const handleSave = (data: AuditCommitteeFormValues) => {
        console.log("Saving draft:", data);
        toast({ title: 'Draft Saved', description: 'Your changes have been saved.' });
    };

    const handleMarkComplete = (data: AuditCommitteeFormValues) => {
        ...
        toast({ title: 'Complete', description: 'Audit Committee minutes have been finalized and uploaded to DMS (simulated).' });
    };
    ...
    const renderEditablePersonnelTable = () => (
        <Card>
            <CardHeader><CardTitle>Audit Committee Personnel</CardTitle><CardDescription>Add the personnel who attended the meeting.</CardDescription></CardHeader>
            <CardContent>
                <Table>
                    <TableHeader><TableRow><TableHead>Name</TableHead><TableHead className="w-[120px]">Actions</TableHead></TableRow></TableHeader>
                    <TableBody>
                        {personnelFields.map((field, index) => (
                             editingPersonnelIndex === index ? (
                                 <TableRow key={field.id}>
                                    <TableCell><Input value={newPersonnel.name || ''} onChange={e => setNewPersonnel(prev => ({ ...prev, name: e.target.value }))} /></TableCell>
                         ...
                         {editingPersonnelIndex === null && (
                            <TableRow>
                                <TableCell><Input placeholder="Personnel Name" value={newPersonnel.name || ''} onChange={e => setNewPersonnel(prev => ({ ...prev, name: e.target.value }))} /></TableCell>
                                <TableCell className="flex gap-1">
                                    <Button size="icon" variant="ghost" onClick={() => { appendPersonnel({ id: uuidv4(), ...newPersonnel }); setNewPersonnel({}); }}><Check className="h-4 w-4 text-green-500" /></Button>
    ...
    const renderEditableMinutesTable = () => (
        <Card>
            <CardHeader><CardTitle>Minutes of Discussion</CardTitle><CardDescription>Capture key issues raised and the committee's response.</CardDescription></CardHeader>
            <CardContent>
                <Table>
                    <TableHeader><TableRow><TableHead>Issues Raised</TableHead><TableHead>Audit Committee's Response</TableHead><TableHead className="w-[120px]">Actions</TableHead></TableRow></TableHeader>
                    <TableBody>
                        {minuteFields.map((field, index) => (
                             editingMinuteIndex === index ? (
                                 <TableRow key={field.id}>
                                    <TableCell><Textarea value={newMinute.issue || ''} onChange={e => setNewMinute(prev => ({ ...prev, issue: e.target.value }))} /></TableCell>
                                ...
                                <TableRow key={field.id}>
                                    <TableCell>{(field as any).issue}</TableCell>
                         ...
                         {editingMinuteIndex === null && (
                            <TableRow>
                                <TableCell><Textarea placeholder="Issue" value={newMinute.issue || ''} onChange={e => setNewMinute(prev => ({ ...prev, issue: e.target.value }))} /></TableCell>
