...
export default function QualityControlReviewPage() {
    ...
    const handleAction = async (action: 'rework-gh' | 'approve-gh') => {
        ...
        try {
            ...
            toast({
                title: 'Success',
                description: `Note has been successfully ${action === 'rework-gh' ? 'sent back to Group Head' : 'approved and submitted to Group Head'}.`
            });
        ...
        <div className="flex h-full w-full flex-col">
            <header className="flex h-auto items-center justify-between gap-4 border-b bg-background p-4 sm:px-0 flex-wrap">
                <div className="flex-1">
                     <h1 className="text-xl font-semibold text-foreground">
                        Quality Control Review: {note.companyName}
                    </h1>
            ...
            <main className="flex-1 pt-6">
                <Suspense fallback={<Skeleton className="h-[calc(100vh-250px)] w-full" />}>
                   <RatingNoteEditor 
                        ref={editorRef} 
                        isReadOnly={true} // QC is a read-only role
